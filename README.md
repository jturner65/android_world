...
**Note on Model Cost:** The `minimal_task_runner.py` script uses a legacy model `gpt-4o-mini` by default. This model can be expensive. For serious usage, you can switch to a more cost-effective model, by modifying the `model_name` in the script.
